...
int main(int argc, char* argv[]) 
{
    ...
    if(outputFlag  == 1)
    {
        ...
        profileOut.output(uN, outputIndex, outputFileNamePrefix, outputFormat);
        profileOut.output(uExact, outputIndex, outputExactFileNamePrefix, outputFormat);
        printf("The discrete L2 norm of the error vector of length %3ld at time %10.5f is %10.5f \n", M+1, time, uErr_norm);
...
//
// This routine advances the solution to u_t + a u_x = 0 one time, timestep using
...
//
	i = 0;
	uOut(i) = uIn(i) - (a*k)*((uIn(i)-uIn(M-1))/h);
//
//  Interior points
//
	for(i = 1; i <= M; i++)
	{
		uOut(i) = uIn(i) - (a*k)*((uIn(i)-uIn(i-1))/h);
	}
}
