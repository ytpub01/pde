...
int main(int argc, char* argv[]) 
{
    ...
    cout << " Enter number of grid panels N : " << endl;
    cin >> N;
...
//
	i = 0;
	uOut(i) = 0.5*(uIn(i+1) + uIn(M-1)) - a*k*(uIn(i+1)-uIn(M-1))/(2.0*h);
///
//  Right edge point --- use periodicity
//
        i = M;
	uOut(i) = 0.5*(uIn(1) + uIn(i-1)) - a*k*(uIn(1)-uIn(i-1))/(2.0*h);
//
//  Interior points
//
	for(i = 1; i <= M-1; i++)
	{
		...
		uOut(i) = 0.5*(uIn(i+1) + uIn(i-1)) - a*k*(uIn(i+1)-uIn(i-1))/(2.0*h);
	}
}
