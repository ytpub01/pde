...
//
	i = 0;
	uOut(i) = 0;
//	uOut(i) = uIn(i) - (a*k)*(uIn(i+1) - 2.0*uIn(i) + uIn(M-1))/pow(h,2);
///
//  Right edge point --- use boundary condition
//
        i = M;
	uOut(i) = 0;
//	uOut(i) = uIn(i) - (a*k)*(uIn(1) - 2.0*uIn(i) + uIn(i-1))/pow(h,2);
//
...
//	z(i-1) = uIn(i) - (a*k)*(uIn(i+1) - 2.0*uIn(i) + uIn(i-1))/pow(h,2);
...
//
//  Add boundary conditions to z, but here zeros
//
...
double maxNorm(DoubleArray1D& uIn, double h)
{
	...
	for (i = 0; i <= M; i++)
	{
        printf(" result = %10.5f uIn = %10.5f fabs(uIn) = %10.5f ", result, uIn(i), fabs(uIn(i)));
		if (fabs(uIn(i)) >= result)
			result = fabs(uIn(i));
        printf(" so result = %10.5f \n", result);
//		result += pow(uIn(i), 2);	
	}
