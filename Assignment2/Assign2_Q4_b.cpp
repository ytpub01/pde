...
//
        i = 0;
        uOut(i) = uIn(i) - (a*k)*((1.5*uIn(i+1)-2.0*uIn(i)+0.5*uIn(M-1))/h);
//
//  Right edge point --- use periodicity
//
        i = M;
        uOut(i) = uIn(i) - (a*k)*((1.5*uIn(1)-2.0*uIn(i)+0.5*uIn(i-1))/h);
//
//  Interior points
//
        for(i = 1; i <= M-1; i++)
        {
                uOut(i) = uIn(i) - (a*k)*((1.5*uIn(i+1)-2.0*uIn(i)+0.5*uIn(i-1))/h);
        }
}
