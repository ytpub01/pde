...
int main(int argc, char* argv[]) 
{
    ...
    uErr.initialize(M+1);
    uErr = uN - uExact;
    double uErr_norm = discreteL2Norm(uErr, dx);
    printf("The discrete L2 norm of the error vector of length %3ld is %10.5f \n", M+1, uErr_norm);
...
//
        i = 0;
        uOut(i) = uIn(i) - (a*k)*((1.5*uIn(i)-2.0*uIn(M-1)+0.5*uIn(M-2))/h);
        i= 1;
        uOut(i) = uIn(i) -  (a*k)*((1.5*uIn(i)-2.0*uIn(i-1)+0.5*uIn(M-1))/h);
//
//  Interior points
//
        for(i = 2; i <= M; i++)
        {
                uOut(i) = uIn(i) - (a*k)*((1.5*uIn(i)-2.0*uIn(i-1)+0.5*uIn(i-2))/h);
        }
}
