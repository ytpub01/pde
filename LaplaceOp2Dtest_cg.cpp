...
// 
// The i array index (x-direction) runs from 0 to mPanel and
// the j array index (y-dicretion) runs from 0 to nPanel. 
// 
//
// This program demonstrates the use of the two dimensional plot
// capabilities of CAMgraphics, as well as demonstrating the use 
// of utility functions for creating files containing 2D data 
// that can be read and plotted by GNUplot or Matlab.
//
...
//
    ...
    G.surface(Vin.getDataPointer(),mPanel+1,nPanel+1);
    G.frame();
    G.contour(Vin.getDataPointer(),mPanel+1,nPanel+1);
    G.frame();
//
...
/*
//  To view the plot, go to the directory containging the 
